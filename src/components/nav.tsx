    ...
    <nav aria-label="Navigation principale" className="fixed inset-x-0 top-0 z-50">
      <div className="nav-inner px-(--page-gutter) py-4">
        <div className="flex justify-center">
          <div className="hidden items-center gap-1 border border-text-primary/10 bg-surface/88 p-1.5 backdrop-blur-xl md:flex">
            {links.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="px-5 py-3 font-sans text-[0.6875rem] font-semibold uppercase tracking-[0.24em] text-text-primary/82 transition-colors duration-150 hover:text-forest-light"
              >
                {link.label}
              </a>
            ))}
          ...
          >
            <span className="font-sans text-[0.625rem] font-semibold uppercase tracking-[0.24em] text-text-primary">
              {open ? "Fermer" : "Menu"}
            </span>
            <span
          ...
          <div className="grid w-full max-w-sm gap-2 border border-text-primary/10 bg-surface-low/96 p-2 backdrop-blur-xl">
            {links.map((link) => (
              <a
                ...
                className="flex justify-center px-5 py-4 font-sans text-xs font-semibold uppercase tracking-[0.24em] text-text-primary transition-colors duration-150 hover:text-forest-light"
              >
                {link.label}
              </a>
            ))}
